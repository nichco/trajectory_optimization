from rotors.pitt_peters_rotor_surrogate_lift import sm_ct, sm_cp
import csdl
import python_csdl_backend
import numpy as np

class liftRotorModel(csdl.Model):
    def initialize(self):
    ...
    def define(self):
        ...
        cruise_ct, cruise_cp = csdl.custom(vAxial,vTan, op=liftRotorExplicit(name=name,num_nodes=n))

        self.register_output(name+'ct', cruise_ct)
        self.register_output(name+'cp', cruise_cp)

class liftRotorExplicit(csdl.CustomExplicitOperation):
    def initialize(self):
        self.parameters.declare('name',types=str)
        self.parameters.declare('num_nodes')
    def define(self):
