...
N = 9
xta = np.linspace(-100,100,N)/((1000/60)*2.5)
xtb = np.linspace(-100,100,N)/((1000/60)*2.5)
xt = np.zeros((N*N,2))
index = 0
for i in range(N):
    for j in range(N):
...
xlimits = np.array([[-100.0, 100.0], [-100.0, 100.0]])/((1000/60)*2.5)
...
if __name__ == '__main__':
    ...
    num = 100
    x = np.linspace(-100,100,num)/((1000/60)*2.5)
    y = np.linspace(-100,100,num)/((1000/60)*2.5)
    X, Y = np.meshgrid(x, y)
    ...
    levelscp = np.arange(0.05, 0.45, 0.005)
